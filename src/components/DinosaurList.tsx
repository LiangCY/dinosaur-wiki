...
const DinosaurList: React.FC<DinosaurListProps> = ({ onViewDetail }) => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(
        (dinosaur) =>
          ...
          dinosaur.scientific_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
          (dinosaur.description && dinosaur.description.toLowerCase().includes(searchQuery.toLowerCase()))
      );
    }

    // 按时期筛选
    if (selectedPeriod !== 'all') {
      filtered = filtered.filter((dinosaur) => dinosaur.period === selectedPeriod);
    }
        ...
        <Row gutter={[16, 16]} align="middle">
          ...
          <Col xs={24} sm={24} md={8} lg={8}>
            <div className="filter-info flex items-center justify-between">
              ...
              {(searchQuery || selectedPeriod !== 'all' || selectedDiet !== 'all') && (
